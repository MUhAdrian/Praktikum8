...
public abstract class BaseModel<T> {
    ...
    protected boolean insert(T object){
        List<String> fieldsWhitoutId = fields.stream()
            .filter(field -> !field.equalsIgnoreCase("id"))
            .collect(Collectors.toList());

        String fieldNames = String.join(",", fieldsWhitoutId);
        String placeholders = String.join(",", fieldsWhitoutId.stream().map(f -> "?").toArray(String[]::new ));
    ...
    protected boolean update(T object){
        List<String> fieldsWithoutId = fields.stream()
            .filter(fields -> !fields.equalsIgnoreCase("id"))
            .collect(Collectors.toList());
